...
template class eastl::unordered_map<int, int>;
template class eastl::unordered_multimap<int, int>;
template class eastl::unordered_set<Align32>;
template class eastl::unordered_multiset<Align32>;
template class eastl::unordered_map<Align32, Align32>;
template class eastl::unordered_multimap<Align32, Align32>;
...
int TestUnordered()
{   
	EASTLTest_Printf("TestUnordered\n");
	...
	{   // Test unordered_set with cached hash code.
		...
		typedef unordered_set<int, hash<int>, equal_to<int>, EASTLAllocatorType, true> HashSetIntC;
...

	{
		...
		insert_iterator< unordered_set<int> > ii(hashSet1, hashSet1.begin());
		for(int j = 0; j < 1000; j++)
			*ii++ = j;
	}
	...
	{   // Test unordered_map
		...
		// Aligned objects should be CustomAllocator instead of the default, because the 
		// EASTL default might be unable to do aligned allocations, but CustomAllocator always can.
		unordered_map<Align32, int, eastl::hash<Align32>, eastl::equal_to<Align32>, CustomAllocator> hashMap;
		const int kCount = 10000;
	...
	{   
		...
		typedef unordered_set<string> HashSetString;
...

	{
		...
		{
			// For simplicity we duplicate the HashtableValue::mData member as the hash map key.
			typedef unordered_map<eastl_size_t, HashtableValue, HashtableValueHash, HashtableValuePredicate> HashMap;
			HashtableValue value;
		...
		{
			// For simplicity we duplicate the HashtableValue::mData member as the hash map key.
			typedef unordered_multimap<eastl_size_t, HashtableValue, HashtableValueHash, HashtableValuePredicate> HashMap;
			HashtableValue value;
	...
	{
		typedef eastl::unordered_multiset<int> HashMultisetInt;
	...
	{ 
		// Regression of compiler warning reported by Jeff Litz/Godfather regarding 
		// strict aliasing (EASTL 1.09.01) December 2007).
		typedef eastl::unordered_multimap<uint32_t, uint32_t*> Map;
		Map* pMap = new Map;
		delete pMap;
	}
	...
	{ 
		// Regression of user-reported crash.
		eastl::unordered_map<int, eastl::string*>* _hmTextureList;
		_hmTextureList = new eastl::unordered_map<int, eastl::string*>();
		eastl::string* a = NULL;
		(*_hmTextureList)[0] = a;
		delete _hmTextureList;
	}
	...
	{
		// Regression of user-reported Android compiler error.
		typedef eastl::unordered_multimap<HashRegressionA*, HashRegressionB> HMM;
		HMM m_hash;
	...
	{
		// Regression of user-reported GCC 4.8 compile failure.
		typedef eastl::unordered_map<int64_t, Struct> AuditByBlazeIdMap;
