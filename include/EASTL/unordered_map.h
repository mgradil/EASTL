...
namespace eastl
{
	...
	/// EASTL_UNORDERED_MAP_DEFAULT_NAME
	///
	/// Defines a default container name in the absence of a user-provided name.
	///
	#ifndef EASTL_UNORDERED_MAP_DEFAULT_NAME
		#define EASTL_UNORDERED_MAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " unordered_map" // Unless the user overrides something, this is "EASTL unordered_map".
	#endif


	/// EASTL_UNORDERED_MULTIMAP_DEFAULT_NAME
	///
	/// Defines a default container name in the absence of a user-provided name.
	///
	#ifndef EASTL_UNORDERED_MULTIMAP_DEFAULT_NAME
		#define EASTL_UNORDERED_MULTIMAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " unordered_multimap" // Unless the user overrides something, this is "EASTL unordered_multimap".
	#endif


	/// EASTL_UNORDERED_MAP_DEFAULT_ALLOCATOR
	///
	#ifndef EASTL_UNORDERED_MAP_DEFAULT_ALLOCATOR
		#define EASTL_UNORDERED_MAP_DEFAULT_ALLOCATOR allocator_type(EASTL_UNORDERED_MAP_DEFAULT_NAME)
	#endif

	/// EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR
	///
	#ifndef EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR
		#define EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR allocator_type(EASTL_UNORDERED_MULTIMAP_DEFAULT_NAME)
	#endif
	...
	/// unordered_map
	///
	/// Implements a unordered_map, which is a hashed associative container.
	...
	public:
		...
		typedef unordered_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>      this_type;
	...
	public:
		...
		/// Default constructor.
		///
		explicit unordered_map(const allocator_type& allocator = EASTL_UNORDERED_MAP_DEFAULT_ALLOCATOR)
		...
		unordered_map(ForwardIterator first, ForwardIterator last, size_type nBucketCount = 0, const Hash& hashFunction = Hash(), 
				 const Predicate& predicate = Predicate(), const allocator_type& allocator = EASTL_UNORDERED_MAP_DEFAULT_ALLOCATOR)
	...
	/// unordered_multimap
	///
	/// Implements a unordered_multimap, which is the same thing as a unordered_map 
	/// except that contained elements need not be unique. See the 
	/// documentation for unordered_set for details.
	///
	template <typename Key, typename T, typename Hash = eastl::hash<Key>, typename Predicate = eastl::equal_to<Key>,
			  typename Allocator = EASTLAllocatorType, bool bCacheHashCode = false>
	...
	public:
		...
		typedef unordered_multimap<Key, T, Hash, Predicate, Allocator, bCacheHashCode>     this_type;
		typedef typename base_type::size_type                                         size_type;
		typedef typename base_type::key_type                                          key_type;
		typedef T                                                                     mapped_type;
		typedef typename base_type::value_type                                        value_type;     // Note that this is pair<const key_type, mapped_type>.
	...
	public:
		...
		/// Default constructor.
		///
		explicit unordered_multimap(const allocator_type& allocator = EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR)
			: base_type(0, Hash(), mod_range_hashing(), default_ranged_hash(), 
						Predicate(), eastl::use_first<eastl::pair<const Key, T> >(), allocator)
		{
			// Empty
		...
		explicit unordered_multimap(size_type nBucketCount, const Hash& hashFunction = Hash(), 
							   const Predicate& predicate = Predicate(), const allocator_type& allocator = EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR)
			: base_type(nBucketCount, hashFunction, mod_range_hashing(), default_ranged_hash(), 
						predicate, eastl::use_first<eastl::pair<const Key, T> >(), allocator)
		{
			// Empty
		...
		unordered_multimap(const this_type& x)
		  : base_type(x)
		{
		}
		...
		#if EASTL_MOVE_SEMANTICS_ENABLED
			unordered_multimap(this_type&& x)
			  : base_type(eastl::move(x))
			{
			}


			unordered_multimap(this_type&& x, const allocator_type& allocator)
			  : base_type(eastl::move(x), allocator)
			{
			}
		#endif
		...
		///
		/// initializer_list-based constructor. 
		/// Allows for initializing with brace values (e.g. unordered_multimap<int, char*> hm = { {3,"c"}, {3,"C"}, {4,"d"} }; )
		///     
		...
		///
		template <typename ForwardIterator>
		unordered_multimap(ForwardIterator first, ForwardIterator last, size_type nBucketCount = 0, const Hash& hashFunction = Hash(), 
					  const Predicate& predicate = Predicate(), const allocator_type& allocator = EASTL_UNORDERED_MULTIMAP_DEFAULT_ALLOCATOR)
			: base_type(first, last, nBucketCount, hashFunction, mod_range_hashing(), default_ranged_hash(), 
						predicate, eastl::use_first<eastl::pair<const Key, T> >(), allocator)
		{
	...
	inline bool operator==(const unordered_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>& a, 
						   const unordered_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>& b)
	{
		typedef typename unordered_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator const_iterator;
	...
	template <typename Key, typename T, typename Hash, typename Predicate, typename Allocator, bool bCacheHashCode>
	inline bool operator==(const unordered_multimap<Key, T, Hash, Predicate, Allocator, bCacheHashCode>& a, 
						   const unordered_multimap<Key, T, Hash, Predicate, Allocator, bCacheHashCode>& b)
	{
		typedef typename unordered_multimap<Key, T, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator const_iterator;
